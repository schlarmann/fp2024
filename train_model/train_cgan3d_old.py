...
def build_discriminator():
    ...
    disc = layers.Conv3D(128, kernel_size, activation='tanh', padding='same')(merge)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3DTranspose(128, kernel_size, strides=stride_transpose, padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3D(256, kernel_size, activation='tanh', padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3D(256, kernel_size, activation='tanh', padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3D(256, kernel_size, activation='tanh', padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3D(128, kernel_size, activation='tanh', padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3D(64, kernel_size, strides=stride_transpose, activation='tanh', padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3D(32, kernel_size, strides=stride_transpose, activation='tanh', padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
    disc = layers.Conv3D(16, kernel_size, strides=stride_transpose, activation='tanh', padding='same')(disc)
    disc = layers.LeakyReLU(alpha=alpha)(disc)
...
def build_generator():
    ...
    # load labels
    in_label = keras.layers.Input(shape=(antenna_labels,))
    n_nodes = antenna_wires//4 * data_points_per_wire * data_per_point

    #Expanding class-label embedding
    label = layers.Dense(n_nodes)(in_label)

    #Converting flat array as 2d image 
    label = layers.Reshape((antenna_wires//4, data_points_per_wire, data_per_point, 1))(label)

    in_latent = keras.layers.Input(shape=(latent_dim,))
    n_nodes = latent_dim * antenna_wires//4 * data_points_per_wire * data_per_point
    ...
    gen = layers.Reshape((antenna_wires//4, data_points_per_wire, data_per_point, latent_dim))(gen)
    ...
    gen = layers.Conv3DTranspose(64, kernel_size, strides=stride_transpose, padding='same')(merge)
    gen = layers.LeakyReLU(alpha=alpha)(gen)
    gen = layers.Conv3D(128, kernel_size, activation='tanh', padding='same')(gen)
    gen = layers.LeakyReLU(alpha=alpha)(gen)
    gen = layers.Conv3D(128, kernel_size, activation='tanh', padding='same')(gen)
    gen = layers.LeakyReLU(alpha=alpha)(gen)
    gen = layers.Conv3DTranspose(64, kernel_size, strides=stride_transpose, padding='same')(gen)
    gen = layers.LeakyReLU(alpha=alpha)(gen)
    gen = layers.Conv3D(128, kernel_size, activation='tanh', padding='same')(gen)
    gen = layers.LeakyReLU(alpha=alpha)(gen)
